...
c3annots <- read.gmt("data/c3.all.v2023.2.Hs.symbols.gmt")
...
annotations <- list(c2 = c2annots, c3 = c3annots, h = hannots, c4 = c4annots, c5 = c5annots, c6 = c6annots, c7 = c7annots, c8 = c8annots)
...
c3signif_res <- enrichmen_res[["c3"]]$MSigDBpathways %>%
  arrange(p.adj) %>%
  filter(p.adj <= 0.05) 
# No significant enriched terms
...
# Create a bar plot
c4p <- ggplot(c4signif_res, aes(x = reorder(row.names(c4signif_res), p.adj), y = stat, fill = p.adj)) +
...
ggsave(plot = c4p, filename = file.path(resultsDir,"/rerconverge/barplot_c4.png"), dpi = 300, units = "in",width = 8, height = 8)

# Create a scatterplot
top_terms <- head(c4signif_res, 5)
c4p2 <- ggplot(enrichmen_res[["c4"]]$MSigDBpathways, aes(x = stat, y =  -log10(p.adj), color = p.adj <= 0.05)) +
  ...
  labs(x = "Statistic score", y = "-log10(Adjusted P-Value)") +
  scale_size_continuous(range = c(2, 10)) +  # Adjust the size range as needed
  theme_minimal() +
  guides(color = "none") #+
  #ggtitle("Enrichment Analysis Results")
ggsave(plot = c4p2, filename = file.path(resultsDir,"/rerconverge/scatterplot_c4.png"), dpi = 300, units = "in",width = 8, height = 8)
...
# Visualize data 
# Create a bar plot
p <- ggplot(c7signif_res, aes(x = reorder(row.names(c7signif_res), p.adj), y = stat, fill = p.adj)) +
  geom_bar(stat = "identity") +
  labs(x = "Gene Set", y = "Enrichment Score") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ...
  #ggtitle("Enrichment Analysis Results") +
  scale_y_continuous(expand = c(0, 0))
ggsave(plot = p, filename = file.path(resultsDir,"/rerconverge/barplot_c7.png"), dpi = 300, units = "in",width = 8, height = 8)

# Create a scatterplot
top_terms <- head(c7signif_res, 5)
p2 <- ggplot(enrichmen_res[["c7"]]$MSigDBpathways, aes(x = stat, y =  -log10(p.adj), color = p.adj <= 0.05)) +
  geom_point(aes(size = num.genes), alpha = 0.5) +
  geom_text_repel(data = top_terms, aes(label = row.names(top_terms)), 
                  box.padding = 0.5, point.padding = 0.2, 
                  max.overlaps = Inf, nudge_x = 0.1, nudge_y = 0.1,
  ...
  guides(color = "none") #+
  #ggtitle("Enrichment Analysis Results")
ggsave(plot = p2, filename = file.path(resultsDir,"/rerconverge/scatterplot_c7.png"), dpi = 300, units = "in",width = 8, height = 8)
...
combined_plot <- (c4p + p2) + 
  plot_annotation(tag_levels = 'A')
# Save to a file
ggsave(combined_plot, filename = file.path(resultsDir,"/rerconverge/gsea_cebi_lemu_rerconverge.png"),  dpi = 500)
...
ann <- bitr(unique(rownames(rer_genes)), fromType = "SYMBOL", toType =  c("SYMBOL","ENTREZID"), OrgDb = org.Hs.eg.db)
