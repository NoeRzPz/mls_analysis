...
# Fill missing LQ values based on the mode of the same genus
imputed_LQ <- primates_traits %>%
  mutate(genus = str_replace_all(SPECIES, "\\s\\w+", ""),
         imputed = ifelse(is.na(LQ), "Yes", "No")) %>%
  group_by(genus) %>%
  #checks if there's more than one row in the genus group
  mutate(LQ = ifelse(n() > 1 & is.na(LQ), mean(LQ, na.rm = T), LQ)) %>%
  ungroup() %>%
...
p6 <- gheatmap(circ, imputed_traits2, offset = .8, width = .2, colnames_angle = 95, colnames_offset_y = .05) + 
  scale_fill_gradient2(name = "Median difference",  na.value = "grey50")
