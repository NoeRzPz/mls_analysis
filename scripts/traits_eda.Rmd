...
primates_traits <- primates_traits %>%
  ...
  #select(-label) %>%# Remove the label column after arranging
```
...
```{r}
pruned_primates.tree <- as_tibble(pruned_primates.tree)
# mapping external data to the tree structure,
pruned_primates.tree <- full_join(pruned_primates.tree, primates_traits, by = 'label')
# the tbl_tree object is converted to a treedata object
pruned_primates.tree <- tidytree::as.treedata(pruned_primates.tree)
```
...
```{r}
circ <- ggtree(pruned_primates.tree, layout = "circular") +
  geom_tree(aes(color = family), size = 1.5)   # Optional: Color branches/edges
...
p2
# save plot
ggsave(plot = p1, filename = file.path(resultsDir,"descriptive/ggtree_lines.tiff"), dpi = 300, units = "in",width = 6, height = 6)
```
...
```{r}
internal_nodes <- pruned_primates.tree %>% 
  as_tibble %>%
  select(parent) %>%
  distinct()
```
...
```{r}
tree <- as.phylo(pruned_primates.tree)
...
groups_df <- map_df(names(groups_list), function(x) {
  tibble(group = x, N_spp = length(groups_list[[x]]$tips), associated_spp = list(groups_list[[x]]$tips),Intermediate_subnodes = list(groups_list[[x]]$nodes[-1]))
})
...
```{r}
# Cut tree at internal nodes
Species_included <- list()
for (i in internal_nodes) {
  set.seed(123)
  tree <- cutPhylo(pruned_primates.tree, node = internal_nodes[i])#
  Species_included[[i]] <- tree$tip.label
}
names(Species_included) <- internal_nodes
```
...
# Add LQ info
tree_groups <- left_join(tree_groups, primates_lq, by = c("Species" = "label"))
