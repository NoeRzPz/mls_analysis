...
mammalian_traits <- mammalian_traits  %>%
  #select(-c("Order tree", "Predicted_MLS","dif","color","outliers"))  %>%
  filter(Order == "Primates")
primates_traits <- inner_join(mammalian_traits, spp_family[, -1], by = c("Scientific name" = "query"))

primates_traits <- primates_traits %>%
  mutate(spp_treeformat = str_replace(`Scientific name`, " ", "_"), 
         spp_treeformat = str_replace_all(spp_treeformat, "\\s\\w+", "")) %>%
  mutate(ordering = match(spp_treeformat, pruned_primates.tree$tip.label)) %>%
  arrange(ordering)  %>%
...
anage_mammals <- anage %>% 
  ...
  # Standardize MLS and body mass variables
  mutate(mls_s = scale(`Maximum longevity (yrs)`),
         adultweight_s = scale(`Adult weight (g)`))

# Select only primates
primates_lq <- toga_spp %>% 
  filter(ORDER == "Primates") %>%
  mutate(spp_treeformat = str_replace(SPECIES, " ", "_"),
         spp_treeformat = str_replace_all(spp_treeformat, "\\s\\w+", "")) %>%
...
primates_lq <- left_join(primates_lq, spp_family[, -1], by = c("SPECIES" = "query"))

# Save new df with family info
#write.csv(primates_lq, file.path(dataDir,"primates_lq_family.csv"), row.names = FALSE)

# Find the common names between tree labels and trait df
common_names <- intersect(primates_tree$tip.label, primates_lq$spp_treeformat)

# Print alphabetically order different spp, to check for misspell names
sort(setdiff(primates_tree$tip.label, primates_lq$spp_treeformat))
sort(setdiff(primates_lq$spp_treeformat, primates_tree$tip.label))
...
primates_lq <- primates_lq %>%
  mutate(ordering = match(spp_treeformat, pruned_primates.tree$tip.label)) %>%
  arrange(ordering)  %>%
  filter(!is.na(ordering)) # We keep only spp with representation in the tree
  #select(-spp_treeformat) %>%# Remove the spp_treeformat column after arranging
```
...
```{r}
# Histogram
hist(primates_lq$LQ, xlab = "LQ", main = "Histogram of primates LQ")
...
```{r}
primates_lq_tree2 <- primates_lq$LQ
names(primates_lq_tree2) <- primates_lq$spp_treeformat
...
global_median <- median(primates_lq$LQ, na.rm = T)
#pruned_primates.tree$LQ <- primates_lq_tree2
dotColor <- function(x) {
  ifelse(x > global_median, "blue", "lightblue") 
}
p <- ggtree(tree_df) + 
    geom_tiplab() + hexpand(0.4) + # expanding the plotting area so that tree tip labels fit well
...
```{r}
circ <- ggtree(pruned_primates.tree, layout = "circular")
primates_lq_tree2 <- primates_lq %>% 
  column_to_rownames(var = "spp_treeformat") %>% 
  select(LQ)
...
# probar esto     scale_fill_discrete(
         name="Phyla",
         guide=guide_legend(keywidth = 0.8, keyheight = 0.8, ncol = 1))
p1
# save plot
ggsave(plot = p1, filename = file.path(resultsDir,"descriptive/ggtree.tiff"), dpi = 300, units = "in",width = 6, height = 6)
```
```{r}
# No funciona nada
#df <- data.frame(
#  tip.label = names(primates_lq_tree2),
#  LQ = as.vector(primates_lq_tree2))
#tree_data <- fortify(pruned_primates.tree, data = df)
#ggtree(tree_data, aes(color = LQ)) +
#    scale_color_continuous(low = 'darkgreen', high = 'yellow') +
#   theme(legend.position = "right")
```

We cut a tree for its age

```{r groups by phylogeny cuts}
...
# Add LQ info
tree_groups <- left_join(tree_groups, primates_lq, by = c("Species" = "spp_treeformat"))
...
```{r family distribution}
# Calculate global median and standard deviation
global_median <- median(primates_lq$LQ, na.rm = T)
global_sd <- sd(primates_lq$LQ, na.rm = T) 

primates <- primates_lq %>% 
  ...
  # Dray global median
  geom_hline(yintercept = median(primates_lq$LQ, na.rm = T), linetype = "dashed", color = "grey", size = 1) +
  coord_flip() +
  labs(x = 'Family',
...
primates <- primates_traits %>% 
  ...
  stat_summary(fun.y = median, geom = "errorbar",
               aes(ymax = ..y.., ymin = ..y..), size = 1) +
  # Dray global median
  geom_hline(yintercept = median(primates_traits$pgls_residuals), linetype = "dashed", color = "grey", size = 1) +
  coord_flip() +
  labs(x = 'Family',
    y = 'Maximum Longevity residuals') +
  theme_classic() + 
  labs_pubr() +
...
summary_data <- primates_traits %>%
  ...
  summarize(
    n = n(),
    MLS_Mean = mean(MaxLongevity, na.rm = TRUE),
    MLS_SD = sd(MaxLongevity, na.rm = TRUE),
    min_MLS = min(MaxLongevity, na.rm = TRUE),
    max_MLS = max(MaxLongevity, na.rm = TRUE),
