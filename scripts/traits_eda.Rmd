...
treeFam_ordered <- c("Cercopithecidae","Hominidae","Cebidae","Atelidae","Lemuridae","Cheirogaleidae","Daubentoniidae","Galagidae" ,"Lorisidae")
...
scatterLQkg <- primates_traits %>% 
  ...
  geom_point(size = 2) +
  geom_smooth(
    method = 'loess', #fits a non-linear regression using local polynomial regression, for visualizing complex relationships
    aes(body_masskg, LQ), 
    inherit.aes = FALSE,
    col = "black") +
  labs( x = 'Adult Weight (kg)',
    y = 'LQ') +
  theme_classic() + 
  labs_pubr() +
  theme(
...
# Combining the plots
tree_LQdistribution <- plot_grid(tree_plot, primates1SD, rel_widths = c(0.3, 0.7))#ncol=2,
tree_LQdistribution

# save plot
ggsave(plot = tree_LQdistribution, filename = file.path(resultsDir,"descriptive/fig1_B.tiff"), dpi = 300, units = "in",width = 6, height = 6)
```

```{r, eval=FALSE}
# Assuming you have a tree named `pruned_primates.tree2` and a data frame `primates_traits` with a column `Family` that indicates the family of each species
mrcas <- c()
# Determine the MRCA for each family
...
mrca <- MRCA(pruned_primates.tree, 38,37)
mrcas <- c(mrcas, mrca) 
write(mrcas, file = file.path(dataDir, "/mrcas.txt"))

# Assign family names to specific internal nodes
# Define node labels
nodes <- c(48, 64, 43,46,70,74,75)
names(nodes) <- c("Cercopithecidae", "Hominidae","Cebidae","Atelidae","Cheirogaleidae","Daubentoniidae","Lorisidae")
nodes

# View the labeled tree
tree_tbl <- as_tibble(pruned_primates.tree)
# Apply labels
for (i in seq_along(nodes)) {
  tree_tbl <- tree_tbl %>%
    mutate(label = ifelse(is.na(label) & node == nodes[i], names(nodes[i]), label))
}
...
ggtree(tree) + 
  geom_tiplab() + 
  geom_nodelab()
write.tree(tree, file=file.path(dataDir,"/pruned_primates.treenodes.nwk"))


# Collapse multiple nodes
tree_tbl2 <- as.tibble(tree)

#Get the descendant nodes of the nodes with the above labels
descendant_nodes <- tree_tbl2 %>%
  filter(parent %in% c(48, 64, 43,46,70,74,75)) 
# remove descendant nodes
collapsed_tree_tbl <- tree_tbl %>%
  anti_join(descendant_nodes, by = "node")
collapsed_tree_tbl2 <- collapsed_tree_tbl %>%
  filter(!(node %in% c(1:27,30:34))) 
# convert back to phylo
collapsed_tree <- as.phylo(collapsed_tree_tbl)

collapsed_tree_tbl$node
ggtree(collapsed_tree) + 
  geom_tiplab() + 
  geom_nodelab()

# This would do what I want but does not work
# Or use ggtree for a more customizable plot
#p <- ggtree(pruned_primates.tree)
#collapse(p, node=48)
#p1 <- ggtree(pruned_primates.tree)
#p2 <- collapse(p1, 48) + geom_point2(aes(subset=(node==48)), size=5, shape=21)
#p3 <- collapse(p2, 64) + geom_point2(aes(subset=(node==64)), size=5, shape=21)
#gtr <- ggtree(pruned_primates.tree)
#anno_gtr <- gtr+ geom_text(aes(label=node)) +geom_tiplab()
#collapsed_anno_gtr <- anno_gtr%>% collapse(node=48)
```

No usar esto
```{r eval=FALSE}
# Load phylogenetic tree 
pruned_fam <- read.tree(file.path(dataDir,"pruned_primates.families.nwk"))
# visualize the tree 
tree_plot <- ggtree(pruned_fam) +
  # add symbols colored by family
  geom_tippoint(aes(color = Family)) +
  theme(legend.position = "none") # This hides the legend
...
extreme_spp <- rbind(top1SD_spp, bottom1SD_spp) %>%
  arrange(LQ)

extreme_spp <- extreme_spp %>%
  filter(!(label %in% c("Pygathrix_nemaeus","Macaca_nemestrina","Macaca_fuscata","Sapajus_apella")))

# keep a tree with only the extreme_spp
# Find the common names between tree labels and trait df
common_names <- intersect(pruned_primates.tree$tip.label, extreme_spp$label)

# Prune tree, keeping the species from our trait dataframe, drop tips that are NOT in common_names
extreme_primates.tree <- drop.tip(pruned_primates.tree, setdiff(pruned_primates.tree$tip.label, common_names))

# search the images uid
# macaca_fascicularis through an error, I search uid manually: d98c79cf-4ca5-4af8-91d2-9a3e51617360
#Trachypithecus_francoisi 3b9150ed-4203-4cdb-b2a5-c21654857e5f
extr_accepted_names <- c()
for (i in extreme_primates.tree$tip.label) {
  name <- ggimage::autocomplete_name(i)
  extr_accepted_names <- c(extr_accepted_names, name$match_name)
}


d <- ggimage::phylopic_uid(extreme_primates.tree$tip.label)

# Update your plot with the filtered data
p <- ggtree(extreme_primates.tree) %<+% d + 
...
```{r}
#library(rphylopic)
# with this I download all uids from a species in the database
