...
```{r, eval=FALSE}
my_sp2fam_210727 <- read_delim("data/caas_tool_inputs/my_sp2fam.210727.tab", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
ggplot(family_counts, aes(x=reorder(Var1, Freq), y=Freq)) +
...
bottom
```
We keep only families with top and bottoms and also we keep 2 spp from Cercopithecidae family
...
extreme_LQ <- extreme_LQ %>%
  select(label,code) %>%
  filter(!label %in% c("Macaca_fascicularis","Macaca_fuscata","Nasalis_larvatus","Trachypithecus_francoisi","Homo_sapiens","Cheirogaleus_medius"))
...
# save plot
ggsave(plot = scatterLQ, filename = file.path(resultsDir,"descriptive/scatterLQg.tiff"), dpi = 300, units = "in",width = 6, height = 6)
ggsave(plot = scatterLQkg, filename = file.path(resultsDir,"descriptive/scatterLQkg.tiff"), dpi = 300, units = "in",width = 6, height = 6)
...
# Combining the plots
tree_LQdistribution <- plot_grid(tree_plot, primates_q, rel_widths = c(0.3, 0.7))#ncol=2,
tree_LQdistribution
...
```{r, eval=FALSE}
# This would do what I want (collapse by family) but does not work
p <- ggtree(pruned_primates.tree2, aes(color=group)) + theme(legend.position='none')
p %>% collapse(node= 64) + 
  geom_point2(aes(subset=(node==64)), shape=21, size=5)
```
...
combined_plot <- plot_grid(fig1, figC,  nrow = 2) 

# save plots
ggsave(plot = combined_plot, filename = file.path(resultsDir,paste0("descriptive/figure_1.tiff")), dpi = 300, units = "in",width = 6, height = 6)
```
