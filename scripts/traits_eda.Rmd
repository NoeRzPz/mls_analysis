...
bottom1SD_spp <- primates1SD[["data"]] %>%
  ...
  select(label,SPECIES, LQ)
suggested_names <- ggimage::autocomplete_name("Macaca")suggested_names <- ggimage::autocomplete_name("Macaca")

#suggested_names <- ggimage::autocomplete_name("Macaca")
# Function to get the UID for each species from PhyloPic
...
top_uids <- as.data.frame(t(top_uids))
#Ensure that top_uids does not contain any NA values before plotting.
image_urls <- sapply(top_uids$uid, function(uid) {
  if (!is.na(uid)) {
    return(paste0("http://phylopic.org/assets/images/submissions/", uid, ".png"))
  } else {
    return(NA)
  }
})
#http://phylopic.org/assets/images/submissions/79ad5f09-cf21-4c89-8e7d-0c82a00ce728.png

library(rphylopic)
df <- data.frame(
  url = image_urls,
  y = 1:length(image_urls)
)
ggplot(df, aes(x = 1, y = y, image = url)) +
  geom_image() +
  theme_void()
df <- data.frame(x = rep(1,nrow(top_uids)), y = 1:nrow(top_uids))
ggplot(df) +
  geom_phylopic(aes(x = x, y = y, uuid = uid)) +
  facet_wrap(~uid) 
```
...
add_phylopic_base(name = "Canis lupus", x = 0.75, y = 0.75, ysize = 0.25,verbose = T)
```
...
tree_plot <- ggtree(pruned_primates.tree2) +
  ...
  geom_tiplab(align = TRUE) + xlim(NA, 3)

primates1SD %>% insert_left(tree_plot)
primates <- primates_traits %>% 
  ...
  theme(
    ...
    text = element_text(size = 18))
# Combining the plots
combined_plot <- plot_grid(tree_plot, primates, rel_widths = c(0.3, 0.7))
...
tree_plot+ geom_facet(panel = "LQ", data = df_bar_data, geom = geom_point, 
