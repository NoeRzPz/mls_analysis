...
primates_traits$LQ <- ifelse(primates_traits$LQoutlier == "High_outlier", NA, primates_traits$LQ)
...
Esto no sirve, hacerlo otra forma
...
```{r}
tree0 <- cutPhylo(pruned_primates.tree, age = 0, keep.lineage = T)
tree10 <- cutPhylo(pruned_primates.tree, age = 10, keep.lineage = T) # linajes no cortados cómo se representarían?
```
...
primates <- primates_traits %>% 
  ...
  # Dray global median
  geom_hline(yintercept = median(primates_traits$LQ, na.rm = T), linetype = "dashed", color = "grey", size = 1) +
  coord_flip() +
  labs(x = 'Family',
